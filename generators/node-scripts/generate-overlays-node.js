...
function generateAllOverlays() {
    ...
    console.log('🎨 Starting generation of 200 overlays...');
    console.log(`📁 Output directory: ${overlaysDir}`);
    console.log('');
    ...
    for (let i = 1; i <= 200; i++) {
        try {
            ...
            if (i % 20 === 0) {
                console.log(`✅ Generated ${i} overlays... (${successCount} success, ${errorCount} errors)`);
            }
        } catch (error) {
            errorCount++;
            console.error(`❌ Error generating overlay ${i}:`, error.message);
        }
    }
    
    console.log('');
    console.log('🎉 Generation complete!');
    console.log(`✅ Successfully generated: ${successCount} overlays`);
    if (errorCount > 0) {
        console.log(`❌ Errors: ${errorCount} overlays`);
    }
    ...
    const files = fs.readdirSync(overlaysDir).slice(0, 10);
    files.forEach(file => {
        console.log(`   - ${file}`);
    });
    if (files.length === 10) {
        console.log(`   ... and ${fs.readdirSync(overlaysDir).length - 10} more files`);
    }
}
